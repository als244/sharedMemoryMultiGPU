...
Train_ResNet * init_trainer(int id, ResNet * model, Batch * cur_batch, Shared_Struct * shared_struct, int batch_size, float learning_rate, float weight_decay, float mean_decay, float var_decay, float eps, int n_epochs, cudnnHandle_t * handle, const char * dump_dir){
...
void prepareAndDoActivation(Train_ResNet * trainer, size_t size, float * input, int spatial_dim, int filters, int batch_size, float *output){
	...
	status = cudnnSetTensor4dDescriptor(input_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, filters, spatial_dim, spatial_dim);
	...
	status = cudnnActivationForward(trainer -> cudnnHandle, activation_descriptor, &alpha, input_descriptor, input, &beta, input_descriptor, output);
...
void prepareAndDoActivationDeriv(Train_ResNet * trainer, size_t size, float * input, float * out_layer_deriv, float * output, int spatial_dim, int filters, int batch_size, float *input_deriv){
	...
	cudnnActivationBackward(trainer -> cudnnHandle, activation_descriptor, &alpha, tensor_descriptor, output, tensor_descriptor, out_layer_deriv, tensor_descriptor, input, &beta, tensor_descriptor, input_deriv);
...
void prepareAndDoMaxPool(Train_ResNet * trainer, const float * input, int in_spatial_dim, int filters, int kern_dim, int stride, int batch_size, float * out){
	cudnnStatus_t status;
	...
	cudnnTensorDescriptor_t input_descriptor;
	status = cudnnCreateTensorDescriptor(&input_descriptor);
	status = cudnnSetTensor4dDescriptor(input_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, filters, in_spatial_dim, in_spatial_dim);

	cudnnTensorDescriptor_t out_descriptor;
	status = cudnnCreateTensorDescriptor(&out_descriptor);
	status = cudnnSetTensor4dDescriptor(out_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, filters, out_spatial_dim, out_spatial_dim);

	cudnnPoolingDescriptor_t pool_descriptor;
	cudnnCreatePoolingDescriptor(&pool_descriptor);
	cudnnSetPooling2dDescriptor(pool_descriptor, CUDNN_POOLING_MAX, CUDNN_NOT_PROPAGATE_NAN, kern_dim, kern_dim, kern_dim / 2, kern_dim / 2, stride, stride);

	const float alpha = 1, beta = 0;

	status = cudnnPoolingForward(trainer -> cudnnHandle, pool_descriptor, &alpha, input_descriptor, input, &beta, out_descriptor, out);
	...
	cudnnDestroyTensorDescriptor(input_descriptor);
	cudnnDestroyTensorDescriptor(out_descriptor);
	cudnnDestroyPoolingDescriptor(pool_descriptor);
}

void prepareAndDoMaxPoolDeriv(Train_ResNet * trainer, const float * input, float * output, float * out_layer_deriv, int in_spatial_dim, int filters, int kern_dim, int stride, int batch_size, float * input_deriv){
	cudnnStatus_t status;
	...
	status = cudnnPoolingBackward(trainer -> cudnnHandle, pool_descriptor, &alpha, out_descriptor, output, out_descriptor, out_layer_deriv, input_descriptor, input, &beta, input_descriptor, input_deriv);
...
void prepareAndDoConvolution(Train_ResNet * trainer, int in_spatial_dim, int kern_dim, int in_filters, int out_filters,  int stride, int batch_size, 
																float * input, float * weights, float * output, int * algo){
	cudnnStatus_t status;

	cudnnTensorDescriptor_t input_descriptor;
	status = cudnnCreateTensorDescriptor(&input_descriptor);
	status = cudnnSetTensor4dDescriptor(input_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, in_filters, in_spatial_dim, in_spatial_dim);
	...
	if (*algo == -1){
		...
		status = cudnnGetConvolutionForwardAlgorithmMaxCount(trainer -> cudnnHandle, &max_count);
...
void prepreAndDoConvolutionDeriv(Train_ResNet * trainer, int in_spatial_dim, int kern_dim, int in_filters, int out_filters, int stride, int batch_size, bool toAdd,
												...
												float * input_deriv, float * weight_deriv, bool toComputeInputDeriv, int * dataAlgo, int *filtAlgo){
	...
	cudnnSetTensor4dDescriptor(input_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, in_filters, in_spatial_dim, in_spatial_dim);

	cudnnTensorDescriptor_t output_descriptor;
	cudnnCreateTensorDescriptor(&output_descriptor);
	cudnnSetTensor4dDescriptor(output_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, out_filters, out_spatial_dim, out_spatial_dim);
	...
	if (toComputeInputDeriv){
		...
		cudnnConvolutionBwdDataAlgo_t convolution_data_algorithm = (cudnnConvolutionBwdDataAlgo_t) *dataAlgo;
		status = cudnnGetConvolutionBackwardDataWorkspaceSize(trainer -> cudnnHandle, kernel_descriptor, output_descriptor, convolution_descriptor, input_descriptor, convolution_data_algorithm, &workspace_bytes);
	...
	cudnnConvolutionBwdFilterAlgo_t convolution_filter_algorithm = (cudnnConvolutionBwdFilterAlgo_t) *filtAlgo;
	

	cudnnStatus_t status_bytes = cudnnGetConvolutionBackwardFilterWorkspaceSize(trainer -> cudnnHandle, input_descriptor, output_descriptor, convolution_descriptor, kernel_descriptor, convolution_filter_algorithm, &workspace_bytes);
...
void prepareAndDoBatchNormAndActivate(Train_ResNet * trainer, BatchNorm * batch_norm_params, Cache_BatchNorm * batch_norm_cache, int batch_size, float eps, float * input, float * output, bool to_activate){
	...
	cudnnTensorDescriptor_t input_descriptor;
	cudnnCreateTensorDescriptor(&input_descriptor);
	cudnnSetTensor4dDescriptor(input_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, filters, spatial_dim, spatial_dim);
...
void prepareAndDoActivationAndBatchNormDeriv(Train_ResNet * trainer, BatchNorm * batch_norm_params, Cache_BatchNorm * batch_norm_cache, BatchNorm * batch_norm_param_derivs,
	...
	cudnnSetTensor4dDescriptor(layer_descriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size, filters, spatial_dim, spatial_dim);
...
void forward_pass(Train_ResNet * trainer){
	...
	prepareAndDoConvolution(trainer, init_spatial_dim, init_kernel_dim, init_in_filters, init_out_filters, init_stride, batch_size, input, first_conv, first_conv_output, &conv_algos[conv_algo_ind]);
	conv_algo_ind++;
	...
	prepareAndDoMaxPool(trainer, init_activated, init_spatial_dim / init_stride, init_out_filters, init_maxpool_dim, init_maxpool_stride, batch_size, init_convblock_input);
	...
	for (int i = 0; i < n_conv_blocks; i++){
		...
		prepareAndDoConvolution(trainer, in_spatial_dim, kern_dim, in_filters, out_filters, stride, batch_size, conv_input, conv_weights, conv_output, &conv_algos[conv_algo_ind]);
		conv_algo_ind++;
		...
		if (projection_weights){
			...
			prepareAndDoConvolution(trainer, in_spatial_dim, kern_dim, in_filters, out_filters, stride, batch_size, conv_block_input, projection_weights, transformed_residual, &conv_algos[conv_algo_ind]);
			conv_algo_ind++;
		...
		if (i != 0) {
			cudaFree(conv_block_input);
		}

		printDeviceData("CONV OUTPUT + (TRANSFORMED) RESIDUAL", conv_block_output, print_size);

		cudaMalloc(&conv_block_output_activated, total_size_conv_block_output * sizeof(float));
	...
	cudaFree(conv_block_output_activated);
...
void backwards_pass(Train_ResNet * trainer){
	...
	cudaMalloc(&activ_deriv_buff, max_activation_size * sizeof(float));

	float * block_activ_deriv;
	cudaMalloc(&block_activ_deriv, max_activation_size * sizeof(float));

	float * temp_deriv_buff;
	cudaMalloc(&temp_deriv_buff, max_activation_size * sizeof(float));
	...
	for (int i = n_conv_blocks - 1; i >= 0; i--){
		...
		// cudaMalloc(&temp_conv_out_activated, output_size * sizeof(float));
		...
		//cudaFree(temp_conv_out_activated);

		cudaMemcpy(activ_deriv_buff, block_activ_deriv, max_activation_size * sizeof(float), cudaMemcpyDeviceToDevice);
		...
		if (cur_conv_block_params -> projection){
			...
			//cublasSaxpy(total_size, 1.0, block_activ_deriv, 1, conv_block_input_deriv, 1);
			cudaMemcpy(conv_block_input_deriv, block_activ_deriv, total_size * sizeof(float), cudaMemcpyDeviceToDevice);
	...
	prepreAndDoConvolutionDeriv(trainer, in_spatial_dim, kern_dim, in_filters, out_filters, stride, batch_size, false,
													conv_input, conv_weight, conv_out_deriv,
													conv_input_deriv, conv_weight_deriv, false, &data_algos[conv_algo_ind], &filt_algos[conv_algo_ind]);
	conv_algo_ind++;

	printDeviceData("INIT CONV WEIGHT DERIV", conv_weight_deriv, print_size);

	cudaFree(activ_deriv_buff);
	cudaFree(temp_deriv_buff);
	cudaFree(prev_conv_block_out_deriv);
	cudaFree(block_activ_deriv);
}
...
void * share_grad(void * thread_data_inp){
	...
	cudaMemcpy(&all_shared_values[loc_in_shared], grad_location_local, param_size * sizeof(float), cudaMemcpyDeviceToHost);
...
void * get_reduced_grad(void * thread_data_inp){
	...
	cudaMemcpy(grad_location_local, &all_shared_values[location_ind], param_size * sizeof(float), cudaMemcpyHostToDevice);
}
...
void update_parameters(Train_ResNet * trainer){
	...
	// DOING ALL-REDUCE
	share_grads(trainer, n_locations, param_sizes, current_gradient_locations);
	get_reduced_grads(trainer, n_locations, param_sizes, current_gradient_locations);
...
int main(int argc, char *argv[]) {
	...
	const char * MY_DUMP_DIR = "cudnn_single";

	Train_ResNet * trainer = init_trainer(id, model, batch, shared_struct, BATCH_SIZE, LEARNING_RATE, WEIGHT_DECAY, MEAN_DECAY, VAR_DECAY, EPS, N_EPOCHS, &cudnn, MY_DUMP_DIR);
